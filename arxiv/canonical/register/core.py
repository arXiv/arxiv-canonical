...
def partial_version_uri_to_io(sources: Sequence[ICanonicalSource]) \
    ...
    def _uri_to_io(obj_type: Type[D.Version], obj: D.Version) -> D.Version:
...
class RegisterListing(Base[D.ListingIdentifier,
    ...
    def create(cls, s: ICanonicalStorage, sources: Sequence[ICanonicalSource],
        ...
        r = R.RecordListing.from_domain(d, callbacks=[io_to_uri])
...
class RegisterVersion(Base[D.VersionedIdentifier,
    ...
    def create(cls, s: ICanonicalStorage, sources: Sequence[ICanonicalSource],
        ...
        r = R.RecordVersion.from_domain(d, callbacks=[io_to_uri])
    ...
    def load(cls: Type[_Self], s: ICanonicalStorage,
        ...
        callbacks = [partial_version_uri_to_io(sources)]
        d = R.RecordMetadata.to_domain(stream, callbacks=callbacks)   # self.load_deferred
        ...
        r = R.RecordVersion.from_domain(d, metadata=_r, callbacks=[io_to_uri])
...
class RegisterEPrint(Base[D.Identifier,
    ...
    def _add_events(self, s: ICanonicalStorage,
        ...
        for event in events:
            ...
            added.add(adder(s, sources, event))
        return added
