...
class RecordEntry(Generic[_EDomain]):
    ...
    domain: _EDomain
    stream: D.RecordStream

    def __init__(self, key: Key, stream: D.RecordStream, domain: _EDomain) \
            -> None:
        self.key = key
        self.domain = domain
        self.stream = stream
    ...
    @classmethod
    def to_domain(cls, stream: D.RecordStream,
...
class RecordListing(RecordEntry[D.Listing]):
    ...
    @classmethod
    def from_stream(cls, key: Key, stream: D.RecordStream,
                    callbacks: Iterable[D.Callback] = ()) -> 'RecordListing':
        return cls(key=key, stream=stream,
                   domain=cls.to_domain(stream, callbacks=callbacks))

    @classmethod
    def make_key(cls, identifier: D.ListingIdentifier) -> Key:
