...
class TestLoadVersions(TestCase):
    ...
    def test_add_eprints_from_scratch(self):
        ...
        _, source_path = tempfile.mkstemp(dir=basedir, suffix='.tar.gz')
        with open(source_path, 'wb') as f:
            f.write(b'fakecontent')
        _, pdf_path = tempfile.mkstemp(dir=basedir, suffix='.pdf')
        with open(pdf_path, 'wb') as f:
            f.write(b'fakepdfcontent')

        fs = filesystem.Filesystem(basedir)

        r = Primary(s, [s, fs], mock.MagicMock()).register
