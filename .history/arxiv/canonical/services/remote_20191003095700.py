...
class DeferredReader(io.BytesIO):
    def __init__(self, method: Callable[[D.URI, ...], requests.Response],
    ...
    def read(self, size: Optional[int] = -1) -> bytes:
        if self._reader is None:
            self._reader = ReadWrapper(
                self._method(str(self._uri), stream=self._stream).iter_content
            )
